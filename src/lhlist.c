...
/* functions declared */
#if WIN32
static void print_size P((long packed_size, long original_size));
static void print_stamp P((time_t t));
static void print_bar P((void));
static void list_header P((void));
static void list_one P((register LzHeader *hdr));
static void list_tailer P((void));
#endif /* WIN32 */
/* ------------------------------------------------------------------------ */
/* Print Stuff																*/
/* ------------------------------------------------------------------------ */
...
print_stamp(time_t t)
{
	...
	printf("%02d/%02d/%02d %02d:%02d",
		(p->tm_year + 1900) % 100, p->tm_mon, p->tm_mday, p->tm_hour,
		p->tm_min);
#else
